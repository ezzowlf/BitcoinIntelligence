...
try{
  ...
  [ordered]@{local_commit=$commit;branch=$branch;master_version="MASTER-3.0";config_hash="db4b6ed6e40fd9f6ce7efe4b0748c61dee83ddcf0603962eb836eeafa8e3ee5f";master_hash=$master.master_code_hash;reference_set=$reference.reference_set;reference_hash=$reference.factor_matrix_sha256;deployment_timestamp=$null;package_created_at=(Get-Date).ToUniversalTime().ToString("o");github="NOT_CONFIGURED";execution="DISABLED"}|ConvertTo-Json|Set-Content -LiteralPath (Join-Path $temp "DEPLOYED_VERSION.json") -Encoding utf8
