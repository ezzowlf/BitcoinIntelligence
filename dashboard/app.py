...
st.markdown(f"""<div class='decision'><div><label>MAKRO</label><strong class='positive'>{decision_state['macro']}</strong></div><div><label>LANGFRISTIG</label><strong class='score'>{decision_state['long_swing']}</strong></div><div><label>TIMING</label><strong>{decision_state['timing']}</strong></div><div><label>RISIKO</label><strong class='{"negative" if risk_state=="HIGH" else "score" if risk_state=="CAUTION" else "positive"}'>{risk_state}</strong></div><div><label>AKTUELLES SZENARIO</label><strong>{decision_state['current_scenario']['name']}</strong></div></div><div class='viewbar'><b>LEITSIGNAL (CONTROL 3 / MACRO 7)</b> · {decision_state['why'][0]} · {decision_state['why'][1]} · Ein langfristiges Kaufsignal bleibt gesperrt, solange noch keine ausreichend bestätigte Kaufgelegenheit vorliegt — Details siehe ENTSCHEIDUNGSBEGRÜNDUNG unten.</div>""",unsafe_allow_html=True)
...
<div class='sidepanel' style='margin:0'><h4>ZYKLUS &amp; STRUKTUR</h4><div class='compactlist'><span><b>ZYKLUS</b> {fusion['regime']} · {cycles['current']['drawdown']:.1%} vom Allzeithoch</span><span><b>ELLIOTT PRIMÄR</b> {macro_primary.get('name','Unresolved')}</span><span><b>SZENARIO</b> {active_scenario['name']} · {active_scenario['status']}</span></div></div>
...
with tabs[0]:
    ...
    scenario_rows=[{"Szenario":x["name"],"Status":x["status"],"Warum diese Zone existiert":x["price_zone"]["support"],"Zone":f"${x['price_zone']['low']:,.0f} – ${x['price_zone']['high']:,.0f}","Bestätigung":x["confidence_state"],"Aktiviert wenn":" · ".join(x["activation_conditions"]),"Ungültig wenn":" · ".join(x["invalidation_conditions"]),"Historischer Kontext":" · ".join(x["historical_analogues"]) or "NICHT VERFÜGBAR","Elliott-Kontext":x["elliott_context"]} for x in macro7["scenarios"]]
    st.dataframe(pd.DataFrame(scenario_rows),hide_index=True)
    detail_name=st.selectbox("SZENARIO-DETAILS",[s["name"] for s in macro7["scenarios"]])
...
with tabs[5]:
    ...
    usage=ai.usage_today();health_rows=[
        {"Komponente":"MT5 / BTC-Tick","Status":f"{live['status']} / {tick.get('freshness','OFFLINE')}","Herkunft":"MetaTrader 5"},
