...
wait_labels={"no_confirmed_lower_low":"No new confirmed lower low yet","structure_reclaim":"Structure not yet reclaimed","h4_or_d1_confirmation":"H4/D1 confirmation still missing"};waiting="".join(f"<span>○ {wait_labels.get(x,x)}</span>" for x in md["waiting_for"])
...
<div class='sidepanel' style='margin:0;grid-column:span 2'><h4>DECISION DETAIL <span class='micro'>explains the champion signal above</span></h4><div class='bigstate' style='color:{dec_color}'>{di['decision']}</div><div class='micro'>{decision_explanation['summary']}</div><div class='levelrow'><span>ZONE</span><b>{zone_line}</b></div><div class='levelrow'><span>ENTRY</span><b>{di['entry_status'].replace('_',' ')}</b></div><div class='levelrow'><span>INVALIDATION</span><b>{'—' if di['invalidation_level'] is None else f"${di['invalidation_level']:,.0f}"}</b></div><div class='levelrow'><span>CONFIDENCE</span><b>{di['decision_confidence']['label']} ({di['decision_confidence']['score']}/100)</b></div></div>
<div class='sidepanel' style='margin:0'><h4>HISTORICAL SETUP SCORE <span class='micro' title="A separate SPECIALIST 5 score — how well this setup matches historically successful entries. Not the same measurement as the CONFIDENCE score above.">ⓘ</span></h4><div class='bigstate score'>{m5['buy']['quality']:.0f} / 100</div><div>{m5['buy']['state']} · {m5['buy']['zone_lifecycle']['state']}</div><div class='micro'>Historical match quality, not a probability of success</div></div>
<div class='sidepanel' style='margin:0'><h4>SELL-OFF RISK</h4><div class='bigstate'>{m5['risk']['sell_off_risk']}</div><div class='micro'>{m5['risk']['distribution']} · {m5['risk']['existing_position_action']}</div><div class='levelrow'><span>MAJOR SUPPORT</span><b>{'—' if not support else f'${support["upper_bound"]:,.0f}'}</b></div><div class='levelrow'><span>MAJOR RESISTANCE</span><b>{'—' if not resistance else f'${resistance["lower_bound"]:,.0f}'}</b></div></div>
<div class='sidepanel' style='margin:0'><h4>WAITING FOR</h4><div class='compactlist'>{waiting}</div></div>
<div class='sidepanel' style='margin:0'><h4>CYCLE &amp; STRUCTURE</h4><div class='compactlist'><span><b>CYCLE</b> {fusion['regime']} · {cycles['current']['drawdown']:.1%} from ATH</span><span><b>ELLIOTT PRIMARY</b> {macro_primary.get('name','Unresolved')}</span><span><b>SCENARIO</b> {active_scenario['name']} · {active_scenario['status']}</span></div></div>
<div class='sidepanel' style='margin:0'><h4>RELEVANT EVENTS</h4><div class='compactlist'>{"".join(f"<span>{e.get('importance') or '—'} · {e.headline[:48]}</span>" for _,e in recent_important.tail(2).iterrows()) or "<span>No HIGH/CRITICAL events on record</span>"}</div></div>
</div>""",unsafe_allow_html=True)
