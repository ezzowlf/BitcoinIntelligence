...
CREATE TABLE IF NOT EXISTS decision_alerts(alert_id TEXT PRIMARY KEY,timestamp TEXT NOT NULL,alert_type TEXT NOT NULL,decision TEXT NOT NULL,payload_json TEXT NOT NULL,model_id TEXT NOT NULL,created_at TEXT DEFAULT CURRENT_TIMESTAMP);
...
class ForwardLedger:
    def __init__(self,path): self.path=Path(path);self.path.parent.mkdir(parents=True,exist_ok=True);con=sqlite3.connect(self.path);con.executescript(SCHEMA);con.close()
    def append_snapshot(self,state,commit_hash,config_hash):
        ...
        return payload
    def append_alert(self,alert_id,timestamp,alert_type,decision,payload):
        with sqlite3.connect(self.path) as con: con.execute("INSERT INTO decision_alerts(alert_id,timestamp,alert_type,decision,payload_json,model_id) VALUES(?,?,?,?,?,?)",(alert_id,str(timestamp),alert_type,decision,json.dumps(payload,default=str),"2.3-FROZEN"))
