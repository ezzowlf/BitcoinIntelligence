...
def main():
    parser=argparse.ArgumentParser();parser.add_argument("command",choices=("run","master","snapshot","telegram","health"));parser.add_argument("--telegram-command",default="/decision");parser.add_argument("--dry-run",action="store_true");args=parser.parse_args();current,config=state();ledger=ForwardLedger(root/"database"/"forward_validation.db")
