...
def _pid_uptime(pattern: str):
    ...
    cmd = (
        # A service is fronted by a small wrapper whose command line also
        # matches; pick the largest resident process so CPU/RAM describe the
        # real interpreter, not the shim.
...
def _process_usage(pid):
    """Read-only CPU/RAM for one pid. Never raises into the sample loop."""
    if not pid:
        return {}
    import subprocess
    cmd = ("$p=Get-Process -Id %d -ErrorAction SilentlyContinue;"
           "if($p){[Console]::Out.Write([string]$p.WorkingSet64+';'+[string]$p.CPU)}" % int(pid))
    try:
        out = subprocess.run(["powershell", "-NoProfile", "-Command", cmd],
                             capture_output=True, text=True, timeout=20).stdout.strip()
        rss, cpu = out.split(";")
        return {"rss_mb": round(int(rss) / 1e6, 1), "cpu_seconds": round(float(cpu or 0), 1)}
    except Exception:  # noqa: BLE001
        return {}


def _pipeline_metrics():
    """Queue depth, producer/consumer counters, event age and drops."""
    bp = _read_json(RUNTIME / "backpressure.json") or {}
    lanes = {}
    for lane, m in (bp.get("metrics") or {}).items():
        q = (bp.get("queues") or {}).get(lane, {})
        lanes[lane] = {
            "produced": m.get("produced"),
            "consumed": m.get("consumed"),
            "backlog": (m.get("produced") or 0) - (m.get("consumed") or 0),
            "processing_seconds": m.get("processing_seconds"),
            "last_event_age_seconds": m.get("last_event_age_seconds"),
            "max_event_age_seconds": m.get("max_event_age_seconds"),
            "failed": m.get("failed"),
            "queue_size": q.get("size"),
            "queue_capacity": q.get("capacity"),
        }
    return {"lanes": lanes, "dropped": bp.get("dropped") or {},
            "coalesced": bp.get("coalesced") or {}, "severe": bp.get("severe")}
...
def _persistent_mb():
    """Total persistent TAKEOFF growth surface - not just one folder."""
    total = 0
    for base in (ROOT / "runtime", ROOT / "database", ROOT / "logs", ROOT / "exports"):
        if not base.exists():
            continue
...
def sample():
    ...
    row = {
        ...
        "collector_usage": _process_usage(collector_pid),
